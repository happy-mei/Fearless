...
public class Ex18ErrorsTest {
  ...
  @Test void caughtStackOverflow() { ok(new Res("", "Stack overflowed", 0), """
    package test
    Test:Main {sys -> UnrestrictedIO#sys.printlnErr(CapTries#sys#[Void]{Loop!}.err!.msg)}
    Loop: { ![R]: R -> this! }
    """, Base.mutBaseAliases); }
  @Test void caughtDivByZero() { ok(new Res("", "/ by zero", 0), """
    package test
    Test:Main {sys -> UnrestrictedIO#sys.printlnErr(Try#[Int]{+12 / +0}.err!.msg)}
    """, Base.mutBaseAliases); }
  @Test void caughtFearlessError() { ok(new Res("", "\"oh no\"", 0), """
    package test
    Test:Main {sys -> UnrestrictedIO#sys.printlnErr(Try#[Int]{Error.msg "oh no"}.err!.str)}
    """, Base.mutBaseAliases); }
