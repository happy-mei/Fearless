...
public class MIRInjectionVisitor implements GammaVisitor<MIR> {
  ...
  public MIR.Lambda visitLambda(String pkg, E.Lambda e, Map<String, T> gamma) {
    ...
    if (impls.size() == 1) {
      ...
      var canSingleton = ms.isEmpty() && p.meths(Mdf.mdf, it, 0).stream().noneMatch(CM::isAbs);
    ...
    var canSingletonTrait = p.meths(Mdf.mdf, freshDec.toIT(), 0).stream().noneMatch(CM::isAbs);
