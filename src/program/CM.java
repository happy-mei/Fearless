...
public interface CM {
  ...
  default String toStringSimplified(){ return c() + ", " + name()+"["+ sig().gens().stream().map(Id.GX::toStringWithBounds).collect(Collectors.joining(", "))+"]"+"("+sig().ts().stream().map(T::toString).collect(Collectors.joining(", "))+"): "+ sig().ret(); }
